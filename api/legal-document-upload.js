...
// FIX: Disable Realtime to avoid WebSocket error on Node.js 20 in Vercel serverless
...
async function classifyWithAI(text, docTypeHint) {
  ...
  try {
        ...
        const parsed = JSON.parse(raw);
          console.log('[classifier] tokens:', response.usage?.total_tokens, '| doc:', parsed.docType, '| risks:', parsed.confirmedRiskCategories?.length);
        ...
        return {
          ...
          console.warn('[classifier] AI failed, using fallback:', err.message);
...
function crossReferenceFindings(ruleFindings, aiResult) {
  ...
  for (const finding of ruleFindings) {
        ...
        if (aiConfirmed) {
          ...
          // LOW risk NOT confirmed by AI -> filtered out (reduces false positives)
  }
        ...
        const result = await new Promise((resolve, reject) => {
                  form.parse(req, async (err, fields, files) => {
                              if (err) { reject(err); return; }
                                ...
                                // Create Supabase client once per request (inside handler, not at module level)
                                ...
                                for (const file of uploadedFiles) {
                                                ...
                                                const validated    = crossReferenceFindings(ruleFindings, aiResult);
