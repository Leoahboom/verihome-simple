...
async function handleLegalConsultationPayment(session) {
  ...
  if (storedFileKeys.length > 0) {
        try {
                ...
                // Don't fail the whole webhook — order is already saved, confirmation sent
        }
  } else {
...
async function generateAndSendReport({ customerEmail, customerName, packageType, docType, storedFileKeys, record, sessionId }) {
  ...
  DOCUMENT SUMMARY:
  ${allFindings.map(f => `- ${f.fileName} (${f.detectedDocType.toUpperCase()}, ${f.wordCount} words) — ${f.ruleFindings.length} rule-engine findings`).join('\n')}
  ...
  Please provide:
  1. EXECUTIVE SUMMARY (2-3 sentences — should buyer proceed?)
...
async function sendClientConfirmationEmail(email, name, record) {
  ...
  const { error } = await resend.emails.send({
        ...
        <div class="header"><h1 style="margin:0">Verihome NZ</h1><h2 style="margin:8px 0 0;font-weight:normal;opacity:0.9">Payment Confirmed — Report In Progress</h2></div>
        <div class="content">
        ...
        <div class="step done"><div class="step-num">✓</div><div><strong>Payment Confirmed</strong> — Your order has been received</div></div>
        <div class="step active"><div class="step-num">2</div><div><strong>AI Analysis In Progress</strong> — Documents are being analysed by our NZ property law engine</div></div>
        <div class="step pending"><div class="step-num">3</div><div><strong>Report Delivery</strong> — Full analysis will be emailed to you shortly</div></div>
        </div>
        ...
        <div class="footer"><p>Protocol Zero Limited · AI-Powered NZ Property Document Analysis</p></div>
        </body></html>`,
  });
...
async function sendFullReportEmail(email, name, packageType, record, reportHtml, allFindings) {
  ...
  const { error } = await resend.emails.send({
        ...
        <h2 style="margin:8px 0 0;font-weight:normal;opacity:0.9">${packageLabel} — Property Analysis Report</h2>
        </div>
        <div class="content">
        <p>Dear <strong>${name || 'Valued Customer'}</strong>,</p>
        ...
        <div class="footer"><p>Protocol Zero Limited · Verihome NZ · AI-Powered NZ Property Document Analysis<br>
