...
async function sendClientConfirmationEmail(email, name, record) {
  ...
  const { error } = await resend.emails.send({
        ...
        <div class="footer"><p>Protocol Zero Limited Â· AI-Powered NZ Property Document Analysis</p></div>
        </body></html>`,
  });
...
async function sendFullReportEmail(email, name, packageType, record, reportHtml, allFindings) {
  ...
  const { error } = await resend.emails.send({
        ...
        <span class="badge" style="background:#e3f2fd;color:#1565c0">ð ${allFindings.length} Document(s)</span>
        <span class="badge high">ð´ ${highRisks} High Risk</span>
        <span class="badge medium">ð  ${medRisks} Medium Risk</span>
        <span class="badge low">ð¢ ${lowRisks} Low Risk</span>
        ...
        <div class="footer"><p>Protocol Zero Limited Â· Verihome NZ Â· AI-Powered NZ Property Document Analysis<br>
